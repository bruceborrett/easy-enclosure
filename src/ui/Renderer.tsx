...
export const Renderer = () => {
  ...
  const _internalWalls = useRef<Geom3 | null>(null);
  ...
  const doRotatePanZoom = () => {
    if (rotateDelta[0] || rotateDelta[1]) {
      const updated = orbitControls.rotate(
        { controls: control, camera: camera, speed: rotateSpeed },
        rotateDelta,
      );
    ...
    if (panDelta[0] || panDelta[1]) {
      const updated = orbitControls.pan(
        { controls: control, camera: camera, speed: panSpeed },
        panDelta,
      );
      control = { ...control, ...updated.controls };
      panDelta = [0, 0];
    ...
    if (zoomDelta) {
      const updated = orbitControls.zoom(
        { controls: control, camera: camera, speed: zoomSpeed },
        zoomDelta,
      );
      control = { ...control, ...updated.controls };
      zoomDelta = 0;
    ...
    if (zoomToFit) {
      ...
      const updated = orbitControls.zoomToFit({
        controls: control,
        camera: camera,
        entities: renderOptions.current?.entities,
      });
      control = { ...control, ...updated.controls };
      zoomToFit = false;
      updateView = true;
  ...
  const renderModel = async (params: Params, diff: string[]) => {
    ...
    // Internal Walls
    if (checkDeps(diff, internalWallDeps)) {
      console.log("Rendering internal walls");
      if (internalWallParams.length > 0) {
        if (waterProof) {
          mountsPos = [-width / 2, -length / 2, 0];
        } else {
          mountsPos = [-(width + SPACING / 2), -length / 2, 0];
        }
        _internalWalls.current = translate(mountsPos, internalWalls(params));
      }
    }
    ...
    if (_waterProofSeal.current && waterProof)
      result.push(_waterProofSeal.current);
    if (_pcbMounts.current && pcbMountParams.length > 0)
      result.push(_pcbMounts.current);
    if (_internalWalls.current && internalWallParams.length > 0)
      result.push(_internalWalls.current);
